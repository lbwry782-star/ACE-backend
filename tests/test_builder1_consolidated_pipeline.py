...
class TestConsolidatedProductionFlow(unittest.TestCase):
    def test_blank_name_uses_seven_planning_calls(self) -> None:
        ...
        self.assertIn("strategy_stage", stages)
        self.assertIn("slogan_stage", stages)
        self.assertIn("conceptual_stage", stages)
        self.assertEqual(stages.count("strategy_stage"), 1)
        self.assertEqual(stages.count("slogan_stage"), 1)
        self.assertEqual(stages.count("conceptual_selection"), 0)
        self.assertNotIn("strategy_judge", stages)
        self.assertEqual(len([s for s in stages if s in {
    ...
    def test_supplied_name_uses_six_planning_calls(self) -> None:
        stages: List[str] = []
