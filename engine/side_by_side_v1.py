...
5) Reject generic pairs chosen only for silhouette similarity. Every pair must clearly support or depict the Advertising Goal.
...
- REPLACEMENT RULE (STRICT): REPLACEMENT is allowed only if all of the following are true: (a) silhouette_similarity ≥ 85, (b) both main objects are direct subjects of the Advertising Goal, and (c) A and B belong to clearly distinct functional categories. Distinct functional categories means they do not share the same primary functional role, are not product variants of the same type, and do not perform the same real-world task. If A and B are functionally similar, even with high silhouette_similarity, assign a lower silhouette_similarity so that SIDE_BY_SIDE is chosen instead of REPLACEMENT.
- First ensure strong relevance to the Advertising Goal, then maximise silhouette_similarity as much as possible subject to the REPLACEMENT RULE.
