...
def parse_strategy_scan(raw_payload: object, *, product_description: str) -> List[StrategyCandidate]:
    ...
    for item in candidates_raw:
        ...
        support = _norm_text(item.get("briefSupport"))
        ...
        if not all([lens, problem, advantage, support, source, risk]):
            reasons.append("strategy_scan_candidate_incomplete")
        if source not in RELATIVE_ADVANTAGE_SOURCES:
            reasons.append("strategy_scan_invalid_source")
        if risk not in CLAIM_RISKS:
            reasons.append("strategy_scan_invalid_claim_risk")

        sig = f"{_norm_key(problem)}|{_norm_key(advantage)}"
        if sig in exact_sigs:
            reasons.append("strategy_scan_exact_duplicate")
        exact_sigs.add(sig)
        lenses.add(_norm_key(lens))
