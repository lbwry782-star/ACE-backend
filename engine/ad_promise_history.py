"""
Persistent cross-session memory for video planner advertisingPromise strings.

Redis keys (final):
  ACE:AD_PROMISE_HISTORY:<product_hash>   — JSON list of {promise, created_at, session_id, embedding?} (max 40, FIFO)
  ACE:AD_PROMISE_INDEX                    — JSON object mapping product_hash → metadata for listing/debug
  ACE:AD_PROMISE_STATS:<product_hash>    — JSON counters for retention / soft-reset signals

product_hash = sha256_hex(normalize(name) + "||" + normalize(description))  (full 64-char hex, deterministic)
"""
...
def _redis_get_json(key: str) -> Optional[Any]:
    try:
        ...
        logger.warning("VIDEO_PROMISE_HISTORY_REDIS_FAIL op=get err=%s", e)
        return None
...
def _redis_set_json(key: str, value: Any) -> bool:
    try:
        ...
        logger.warning("VIDEO_PROMISE_HISTORY_REDIS_FAIL op=set err=%s", e)
        return False
...
def _cleanup_history_entries(
    ...
    if aged_removed:
        logger.info("VIDEO_PROMISE_HISTORY_AGE_FILTER removed=%s", aged_removed)
    if trimmed_removed:
        logger.info(
            "VIDEO_PROMISE_HISTORY_TRIMMED kept=%s removed=%s",
...
def load_ad_promise_history(product_name: str, product_description: str) -> List[Dict[str, Any]]:
    ...
    if aged_removed or trimmed_removed:
        logger.info(
            "VIDEO_PROMISE_HISTORY_CLEANUP aged_removed=%s trimmed_removed=%s",
            aged_removed,
            trimmed_removed,
        )
...
def get_all_products_with_memory() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not redis_configured():
            logger.info("VIDEO_PROMISE_INDEX_LOADED count=0")
            return []
        r = get_redis()
        idx = _load_promise_index()
        if not idx:
        ...
        for h, meta in list(idx.items()):
            ...
            cnt = len(data) if isinstance(data, list) else int(meta.get("count") or 0)
            out.append(
                {
                    "product_hash": h,
        ...
        if dirty:
            _save_promise_index(idx)
        out.sort(key=lambda row: row.get("product_hash") or "")
        logger.info("VIDEO_PROMISE_INDEX_LOADED count=%s", len(out))
        return out
    except Exception as e:
...
def delete_product_memory_by_hash(product_hash: str) -> bool:
    ...
    if not ph:
        return False
    logger.info("VIDEO_PROMISE_MEMORY_DELETE_BY_HASH hash=%s", ph)
    try:
        from engine.video_jobs_redis import get_redis, redis_configured
...
def delete_product_memory_by_text(product_name: str, product_description: str) -> Tuple[bool, str]:
    ...
    ph = compute_product_hash(product_name, product_description)
    logger.info("VIDEO_PROMISE_MEMORY_DELETE_BY_TEXT hash=%s", ph)
