...
def _builder1_headline_rhyming_substitution_block() -> str:
    return (
        ...
        "13. Do not add extra words before, inside, or after the twisted expression.\n"
        "14. headlineProductName must exactly match productNameResolved (backend-fixed). headlineText is the twisted expression remainder only — do not repeat the product name in headlineText.\n"
        "15. headlineFull must be exactly headlineProductName, one ASCII space, then headlineText. ≤7 words total on headlineFull.\n"
        "16. headlineText must express the advertisingPromise through the visual interaction — not by restating the promise literally.\n"
        "17. Do NOT pick an expression that already contains the object word before substitution.\n"
    )
